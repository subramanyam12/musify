...
const RecentSongs = () => {
      ...
      localStorage.setItem(
        ...
        JSON.stringify(
          song?.nestsearchdata?.map(
            ({ id, image, name, artists, downloadUrl }) => ({
              id,
              image: typeof image === 'object' ? image?.at(-1)?.url : image,
              name,
              artist: artists?.primary?.[0]?.name,
              downloadUrl:
  ...
  return (
    <div className="w-[30vw] max-sm:z-10 max-sm:w-full">
      <div
        className={`mb-3 px-8 max-sm:px-4 max-sm:pt-3 ${recentclose && 'max-sm:hidden'
          } max-sm:absolute max-sm:-bottom-4 max-sm:left-0 max-sm:z-20 max-sm:w-full max-sm:bg-[#000000]`}
      >
        <div
        ...
        <div className="flex w-full py-5 text-center font-bold capitalize text-white">
          <div
            ...
            className={`w-[50%] cursor-pointer py-2 max-sm:cursor-default ${recentplaylistbool
        ...
        <div className="scroll-none flex h-[30vh] w-full flex-col gap-3 overflow-y-auto max-sm:h-[35vh]">
          {loading ? (
            ...
            <>
              {popular?.length ? (
                popular?.map(({ id, image, name, artist, artists, downloadUrl }, i) => (
                  <div
                    key={id}
                    className={`${song?.index === i &&
                      recentplaylistbool &&
                        ...
                        <span className="text-[#ffffff7e]">
                          {artist
                            ? artist?.length > 25
                              ...
                              : artist
                            : artists?.primary?.[0]?.name?.length > 25
                              ? artists?.primary?.[0]?.name?.substring(0, 25) + '...'
